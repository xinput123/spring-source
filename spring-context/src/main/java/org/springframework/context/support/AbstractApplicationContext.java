...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	@Override // 加载或重建 ApplicationContext，将旧的 ApplicationContext 销毁，再重新执行一次初始化操作
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) { // 加锁，防止 refresh() 操作进来
			// 准备工作，记录容器启动时间、标记"已启动"状态
			prepareRefresh();

			// . 这个方法很重要，告诉子类刷新内部 BeanFactory
			// 这步完成后，配置文件就会解析成一个个 Bean 定义，注册到 BeanFactory 中，
			// 当然，这里的 Bean 还没有初始化，只是配置信息都提取出来了，
			...
			try {
				...
				// Reset 'active' flag. 重置 active 标志
				cancelRefresh(ex);
	...
	/**
	 * 初始化 BeanFactory、加载 Bean、注册 Bean。当然这个步骤完成后，Bean 并没有完成初始化
	 */
	...
	/**
	 * Initialize the ApplicationEventMulticaster. 初始化 applicationEventMulticaster
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		// 首先，初始化名字为 conversionService 的 Bean。初始化的动作包装在 beanFactory.getBean(...) 中
		...
		// 到这一步的时候，Spring 已经开始预初始化 singleton beans 了，所以不需要 bean 定义解析、加载、注册。
