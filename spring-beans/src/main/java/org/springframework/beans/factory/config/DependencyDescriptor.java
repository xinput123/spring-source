...
/**
 * Descriptor for a specific dependency that is about to be injected. 即将注入的特定依赖项的描述符
 * Wraps a constructor parameter, a method parameter or a field,
 * allowing unified access to their metadata. 包装构造函数参数、方法参数或字段，允许对其元数据进行统一访问
 *
...
public class DependencyDescriptor extends InjectionPoint implements Serializable {
	...
	private final Class<?> declaringClass; // 当前申明的注入的描述符，它有一个被注入的容器，这个容器就称为申明的这个类

	@Nullable
	private String methodName; // 方法注入，可以为空

	@Nullable
	private Class<?>[] parameterTypes; // 构造器注入，可以为空

	private int parameterIndex;

	@Nullable
	private String fieldName; // 字段注入，可以为空

	private final boolean required; // 对应 @Autowired 注解中的 required 字段

	private final boolean eager; // 是否实时/饥饿的，对应 @Lazy。 如果 Lazy=true，则 eager = false，反之，eager = true
